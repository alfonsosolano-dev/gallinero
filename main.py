...
# =================================================================
st.set_page_config(page_title="CORRAL IA V36.0", layout="wide", page_icon="🚜")
...
def inicializar_db():
    ...
    for n, e in tablas.items():
        ...
        # PARCHE DE SEGURIDAD: Si la tabla existe pero le faltan columnas de versiones nuevas
        if n == "ventas":
            for col in ["unidades INTEGER", "ilos_finale REAL"]:
...
# =================================================================
CONFIG_IA = {
    "Roja": {"puesta": 145, "cons_base": 0.110, "consejo": "Alta puesta. Ojo con el calcio."},
...
def obtener_consejo(seccion):
    dict_c = {
        ...
        "crecimiento": "💡 IA: El pesaje semanal ayuda a detectar enfermedades antes que el ojo."
    }
...
def vista_dashboard(prod, ventas, gastos, lotes):
    ...
    if not lotes.empty:
        ...
        for _, r in lotes.iterrows():
            ...
            cons_lista.append({"Lote": r['id'], "Especie": r['especie'], "Raza": r['raza'], "Edad": f"{edad} días", "Kg/Día": round(c_dia, 3)})
...
def vista_crecimiento(lotes):
    ...
    for _, r in lotes.iterrows():
        with st.expander(f"Lote {r['id']} - {r['raza']}", expanded=True):
            ...
            with col1: img_cam = st.camera_input(f"Cámara {r['id']}", key=f"c_{r['id']}")
            with col2:
                img_file = st.file_uploader("Subir foto historial", type=['jpg','png','jpeg'], key=f"f_{r['id']}")
                f_foto = st.date_input("Fecha de la foto", datetime.now(), key=f"d_{r['id']}")

            img_final = img_cam if img_cam else img_file
            if img_final and st.button(f"Guardar Foto {r['id']}", key=f"b_{r['id']}"):
...
def vista_gastos():
    ...
    with st.form("g"):
        ...
        imp = c1.number_input("Euros €", 0.0); kg = c2.number_input("Kg Pienso", 0.0)
