...
st.set_page_config(page_title="CORRAL MAESTRO ELITE V16.2", layout="wide", page_icon="🐓")
...
st.sidebar.title("🐓 CORRAL ELITE V16.2")
...
if sec == "Dashboard":
    ...
    for nombre, stock in [("Gallinas", st_gal), ("Pollos", st_pol), ("Codornices", st_cod)]:
        if stock < 5: st.error(f"⚠️ Stock crítico Pienso {nombre}: {stock:.1f} kg.")
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Caja Real", f"{bal_caja:.2f}€")
    c2.metric("Ahorro Casa", f"{v_home:.2f}€")
    c3.metric("Beneficio Total", f"{ben_total:.2f}€")
    ...
    st.info(f"🐔 Blanco: **{(obj - timedelta(days=55)).strftime('%d Oct')}** | 🐓 Campero: **{(obj - timedelta(days=90)).strftime('%d Sept')}**")
    ...
    for _, r in lotes.iterrows():
        ...
        st.write(f"**Lote {r['id']} ({r['raza']})** {ya_pone}"); st.progress(porc/100)
    ...
    with st.form("f_prod"):
        ...
        if st.form_submit_button("Guardar Producción"):
            ...
            conn.commit(); conn.close(); st.success("Producción anotada"); st.rerun()

elif sec == "Puesta":
    st.title("🌟 Registro de Primera Puesta")
    with st.form("f_puesta"):
        l_id = st.selectbox("Lote", lotes[lotes['especie']=='Gallinas']['id'].tolist() if not lotes.empty else [])
        f_h = st.date_input("Fecha del primer huevo")
    ...
    with st.form("f_baja"):
        ...
        if st.form_submit_button("Registrar Baja"):
            ...
            conn.commit(); conn.close(); st.error(f"Baja registrada. Pérdida: {perd}€"); st.rerun()
