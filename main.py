...
st.set_page_config(page_title="CORRAL MAESTRO V.FINAL", layout="wide")
...
if menu == "📊 RENTABILIDAD":
    ...
    with tabs[0]:
        ...
        if not df_g.empty: st.plotly_chart(px.pie(df_g, values='importe', names='categoria'))
    ...
    for i, esp in enumerate(["Gallinas", "Pollos", "Codornices"], 1):
        with tabs[i]:
            ...
            if not p_data.empty: st.plotly_chart(px.line(p_data, x='fecha', y='cantidad'))
...
# --- 5. GASTOS ---
    ...
    with st.form("fg"):
        ...
        con = st.text_input("Concepto"); imp = st.number_input("Importe (€)"); kgs = st.number_input("Kilos", 0.0)
...
# --- 7. ADMIN Y EXPORTACIÓN (Aquí está el botón que faltaba) ---
elif menu == "🛠️ ADMIN":
    ...
    if not df.empty:
        ...
        st.download_button(
            ...
            file_name=f"corral_{tab}_{datetime.now().strftime('%Y%m%d')}.xlsx",
...
# --- SECCIONES RESTANTES (PUESTA, VENTAS, ALTA) ---
elif menu == "🥚 PUESTA":
    st.title("🥚 Puesta"); f = st.date_input("Fecha"); rz = st.selectbox("Raza", ["Roja", "Blanca", "Chocolate", "Codorniz"]); can = st.number_input("Cantidad", 1)
    if st.button("Anotar"): 
        c.execute("INSERT INTO produccion (fecha, raza, cantidad, especie) VALUES (?,?,?,?)", (f.strftime('%d/%m/%Y'), rz, can, "Gallinas" if rz != "Codorniz" else "Codornices"))
        conn.commit(); st.rerun()
